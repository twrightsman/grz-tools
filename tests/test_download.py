...
def test_boto_download(
    ...
    # Prepare directories
    submission_id = (
        "submission123"  # Use the same submission ID as in the download method
    )
    ...
    upload_file(
        remote_bucket, temp_fastq_file_path, f"{submission_id}/large_test_file.fastq"
    )
    upload_file(
        remote_bucket, temp_small_file_path, f"{submission_id}/small_test_file.txt"
    )

    # Create a mock S3 bucket
    download_worker = S3BotoDownloadWorker(
        config=config_model, status_file_path=temp_download_log_file_path
    )
    ...
    assert (
        files_dir / "large_test_file.fastq"
    ).exists(), "Fastq file was not downloaded."
    assert (files_dir / "small_test_file.txt").exists(), "Text file was not downloaded."

    # Further assertions can be made here as necessary
    assert (
        calculate_sha256(files_dir / "large_test_file.fastq")
        == temp_fastq_file_sha256sum
    ), "Fastq SHA256 mismatch."
    assert (
