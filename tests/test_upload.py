...
def test_boto_upload(
    ...
    download_file(remote_bucket, "small_test_file.txt", local_tmpdir_path / "small_test_file.txt")
    download_file(remote_bucket, "large_test_file.fastq", local_tmpdir_path / "large_test_file.fastq")

    assert calculate_sha256(local_tmpdir_path / "small_test_file.txt") == temp_small_file_sha256sum
